...
#ifndef FH_PNM_FILE_H
#define FH_PNM_FILE_H
...
static image<uchar> *loadPBM(const char *name) {
    ...
    for (int i = 0; i < height; i++)
        read_packed( imPtr<uchar>(im, 0, i), width, file);
...
static void savePBM(image<uchar> *im, const char *name) {
    ...
    file << "P4\n" << width << " " << height << "\n";
    for (int i = 0; i < height; i++)
        write_packed( imPtr<uchar>(im, 0, i), width, file);
}
...
static image<uchar> *loadPGM(const char *name) {
    ...
    /* read data */
    image<uchar> *im = new image<uchar>(width, height);
    file.read((char *)imPtr<uchar>(im, 0, 0), width * height * sizeof(uchar));
...
static void savePGM(image<uchar> *im, const char *name) {
    ...
    file << "P5\n" << width << " " << height << "\n" << UCHAR_MAX << "\n";
    file.write((char *)imPtr<uchar>(im, 0, 0), width * height * sizeof(uchar));
}
...
static image<rgb> *loadPPM(const char *name) {
    ...
    file.read((char *)imPtr<rgb>(im, 0, 0), width * height * sizeof(rgb));
...
static void savePPM(image<rgb> *im, const char *name) {
    ...
    file << "P6\n" << width << " " << height << "\n" << UCHAR_MAX << "\n";
    file.write((char *)imPtr<rgb>(im, 0, 0), width * height * sizeof(rgb));
}
